...
export default async function handler(req, res) {
  // Debug log to confirm this handler is invoked
  console.log("🟢 [send-jobApplication] handler invoked");
  ...
  try {
    ...
    console.log("Extracted data:", { email, firstName, surname, jobTitle, jobPosition, jobLink });
    ...
    console.log("Transporter configured for user:", process.env.EMAIL_USER);
    ...
    console.log("🟢 [send-jobApplication] email sent, messageId:", info.messageId);

    return res.status(200).json({ success: true, message: "Application email sent successfully" });
  } catch (error) {
    console.error("Error in send-jobApplication handler:", error);
